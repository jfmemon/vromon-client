...
const ManageFlights = () => {
    ...
    useEffect(() => {
        const serviceId = '64a94a4c6b2df0def9f64cc6'; // Replace with the actual service ID

        // Fetch ticket data from the server
        fetch(`https://vromon-server-roan.vercel.app/services/${serviceId}`)
            .then((res) => res.json())
            .then((data) => {
                setTicketsData(data.flights); // Assuming "data" is an object containing a "bus" array with ticket objects
            })
            .catch((error) => {
    ...
    const handleAddFlightTicket = (event) => {
        event.preventDefault();
        const form = event.target;

        const newTickets = {
            img: addMoreTickets.flightImage,
            name: addMoreTickets.flightName,
        ...
        const serviceId = '64a94a4c6b2df0def9f64cc6';

        fetch(`https://vromon-server-roan.vercel.app/services/flights/${serviceId}`, {
            method: 'POST',
            headers: {
    ...
    return (
        <div className='w-full'>
            ...
            <div>
                <form onSubmit={handleAddFlightTicket} className="card-body">
                    <div className='grid lg:grid-cols-2 sm:grid-cols-1'>
                        <div className='flex flex-col'>
                            <div className="form-control">
                                ...
                                <input onBlur={handleInput} name='flightName' type="text" className="input input-bordered w-3/4" required />
                            </div>
                            <div className="form-control">
                                <label className="label">
                        ...
                        <div className='flex flex-col'>
                            ...
                            <div className="form-control">
                                <label className="label">
                                    <span className="label-text">Flight image url</span>
                                </label>
                                <input onBlur={handleInput} name='flightImage' type="url" className="input input-bordered w-3/4" placeholder="https://example.com" required />
                            </div>
                        </div>
                    </div>
            ...
            <div className='table-responsive'>
                <div className='table-responsive'>
                    <h3 className='text-3xl font-semibold p-3 text-warning'>Stored flights:</h3>
